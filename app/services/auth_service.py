...
from app.dtos.auth_dtos import ForgotPassword, Password, ResetPassword, Login, Token, ExternalLogin
...
def get_user_password(db: Session, username: str) -> Password:
...
def forgot_password(db: Session, forgot_password_data: ForgotPassword) -> None:
...
def reset_password(db: Session, reset_password_data: ResetPassword) -> UserResponse:
...
def get_access_token(db: Session, login_data: Login) -> Token:
...
def get_access_token_for_external_login(db: Session, external_login_data: ExternalLogin) -> Token:
...
def generate_access_token(data: dict) -> Token:
